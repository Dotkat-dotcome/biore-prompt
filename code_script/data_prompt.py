...
class REPromptDataset(DictDataset):
    ...
    def get_labels(self, tokenizer):
        ...
        # for temp_id in self.temps:
            ...
            temp = self.temps[name]['temp'][1]
            _temp = temp.copy()

            # randomly pick up template
            # choice = random.choice([0,1,2])
            # _labels = self.temps[name]['labels'][choice] # _labels = self.temps[name]['labels'][0]
            
            _labels = self.temps[name]['labels'][0]
            _labels_index = []
    ...
    def tokenize(self, item, tokenizer):
        ...
        # e1 = tokenizer.encode((pos_head['name']), add_special_tokens=False)[1:] 
        # e2 = tokenizer.encode((pos_tail['name']), add_special_tokens=False)[1:] 
        ...
        # import ipdb; ipdb.set_trace()
        start = tokenizer.encode(" ".join(self.temps[rel_name]['temp'][0]))[1:-1]
        end = tokenizer.encode(' '+" ".join(self.temps[rel_name]['temp'][2]))[1:-1]
        prompt = start + e1 + self.temp_ids[rel_name]['mask_ids'][0] + end + e2 
        
        # import ipdb; ipdb.set_trace()
        # prompt = self.temp_ids[rel_name]['mask_ids'][0] 
