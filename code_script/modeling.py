...
class Model(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, input_flags, mlm_labels, labels):
        ...
        # hidden_states is the token representations
        # print(hidden_states.shape) # torch.Size([1, 512, 768]) 512 is the max_length
        hidden_states = hidden_states[mlm_labels >= 0].view(input_ids.size(0), len(self.prompt_label_idx), -1)
        # print('mlm_labels', mlm_labels.shape) torch.Size([1, 512])
        # print(hidden_states.shape) # torch.Size([1, 3, 768]) 
